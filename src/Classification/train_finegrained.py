...
def train(epoch, loss='ASYM'):
    ...
    targets, outputs = labels.cpu().detach().numpy().tolist(), np.array(out.cpu().detach().numpy().tolist()) >= 0.5
    f1_score_macro = metrics.f1_score(targets, outputs, average='macro')
    print(f"train F1 Score (Macro) = {f1_score_macro}")
